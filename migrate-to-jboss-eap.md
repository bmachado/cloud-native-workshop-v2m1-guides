...
* Name: `{{ OPENSHIFT_USER_NAME }}-coolstore-dev`
* Display Name: `{{ OPENSHIFT_USER_NAME }} Coolstore Monolith - Dev`
* Description: _leave this field empty_

> NOTE: YOU `MUST` USE `{{ OPENSHIFT_USER_NAME }}-coolstore-dev` AS THE PROJECT NAME, as this name is referenced later on and you will experience failures if you do not name it `{{ OPENSHIFT_USER_NAME }}-coolstore-dev`!
...
2. Enter your username and password assigned to you:
* Username: `{{ OPENSHIFT_USER_NAME }}`
...
4. Copy the oc command showed below "Log in with this token"

![create_new]({% image_path login_with_this_token.png %})

5. Paste it to the terminal window in Code Ready Workspaces
...
Then open up the `{{ OPENSHIFT_USER_NAME }}-coolstore-dev` project status page at [OpenShift web console]({{ CONSOLE_URL}}){:target="_blank"}
